...
| Type | Module | Responsibility |
|------|--------|----------------|
| `RemotePluginManifest` | `core:ui-plugin-sdk` | Cloud plugin declaration: `id` / `version` / `downloadUrl` / `sha256` / `entryClass` (+ `minCoreVersion` / `minSdk` / `signature`). Pure, serializable, no Android dep — shared with plugins so they resolve `UiFormatPlugin` inside their own `ClassLoader`. |
...
| `DynamicPluginManager` | `app` | `DexClassLoader` load from internal storage → reflect `entryClass` → register into `PluginManager`. Hot-updates same-id plugins. |
| `PluginDownloader` | `app` | `HttpURLConnection` download to `filesDir/plugins/<id>/<id>.dex` with SHA-256 cache hit. |
...
| Marketplace UI | `app` | "Market" tab (index 7): browse repo, install/update on demand, list registered plugins. |
